        ...
        <form class="row g-3" method="post" action="/kerusakan-bmn" enctype="multipart/form-data">
            ...
            <div class="form-data">
                ...
                @foreach($dataCreate as $data)
                        ...
                        <input type="file" class="form-control @error($data['name']) is-invalid @enderror " id="{{ $data['name'] }}" name="{{ $data['name'] }}" value="{{ old($data['name']) }}" {{ $data['required'] ? 'required' : ' ' }} accept="image/*"></input>
